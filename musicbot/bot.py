...
class MusicBot(discord.Client):
    def __init__(self, config_file=None, perms_file=None, aliases_file=None):
        try:
            sys.stdout.write("\x1b]2;ChudyBot {}\x07".format(BOTVERSION))
        except:
            pass
    ...
    async def on_ready(self):
        ...
        log.info("Connected as {0}#{1} with ID: {2}".format(
            self.user.name,
        ...
        if owner and self.guilds:
            log.info("Creator:     {0}#{1}    with ID: {2}\n".format(
                owner.name,
                owner.discriminator,
                owner.id
            ))

            log.info('Servers List:')
        ...
        if self.config.missing_keys:
            ...
            log.info("Theoretically working, practically God knows.\n")

    def _gen_embed(self):

        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='Dodek69/ChudyBot {}'.format(BOTVERSION), icon_url='https://files.sharenator.com/22859.jpg')
        e.set_author(name=self.user.name, url='https://github.com/Dodek69/ChudyBot', icon_url=self.user.avatar_url)
        return e
    ...
    async def on_message(self, message):
        ...
        if isinstance(message.channel, discord.abc.PrivateChannel):
            if not (message.author.id == self.config.owner_id and command == 'joinserver'):
                await self.safe_send_message(message.channel, 'Sklej tace.')
                return
